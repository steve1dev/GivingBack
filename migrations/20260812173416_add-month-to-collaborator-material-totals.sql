-- Collaborator impact was only tracked per year, which can't drive a
-- month-over-month comparison chart. Add month granularity and re-key the
-- upsert target so log_recycling_detection() can attribute each event to
-- its actual month.
alter table public.collaborator_material_totals
	add column month smallint;

update public.collaborator_material_totals
set month = extract(month from coalesce(updated_at, created_at, now()))::smallint
where month is null;

alter table public.collaborator_material_totals
	alter column month set not null,
	alter column month set default extract(month from now())::smallint;

alter table public.collaborator_material_totals
	add constraint collaborator_material_totals_month_check
	check (month >= 1 and month <= 12);

alter table public.collaborator_material_totals
	drop constraint collaborator_material_totals_collaborator_id_year_material__key;

alter table public.collaborator_material_totals
	add constraint collaborator_material_totals_collaborator_id_year_month_ma_key
	unique (collaborator_id, year, month, material_category);

create index if not exists idx_collaborator_material_totals_year_month
	on public.collaborator_material_totals (collaborator_id, year, month);
...
begin
	...
	-- collaborator's monthly total (public impact stats).
	select id
	...
	if v_default_collaborator_id is not null then
		...
		insert into public.collaborator_material_totals (collaborator_id, year, month, material_category, quantity_kg)
		values (v_default_collaborator_id, extract(year from v_now)::int, extract(month from v_now)::int, v_rule.material_category, v_kg)
		on conflict (collaborator_id, year, month, material_category) do update

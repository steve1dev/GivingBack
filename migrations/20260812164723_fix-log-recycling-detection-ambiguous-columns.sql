-- The RETURNS TABLE clause implicitly declares OUT parameters named
-- total_points and recycled_at, which shadow the same-named columns on
-- user_points_wallet and recycling_events. Two statements referenced those
-- columns unqualified, so every call raised "column reference is ambiguous"
-- and the insert/points/streak update never committed. Qualify both.
...
begin
	...
	insert into public.recycling_events as re (
		user_id,
		model_class,
		material_category,
		display_label,
	...
	returning re.id, re.recycled_at into v_event_id, v_recycled_at;
	...
	select w.total_points
		into v_total_points
	from public.user_points_wallet as w
